...
describe('Beam utility functions', () => {
  ...
  it('validates control message shape', () => { expect(parseMessage({ type: 'hello', name: 'Phone', deviceType: 'phone' })).not.toBeNull(); expect(parseMessage({ type: 'hello', name: 5 })).toBeNull() })
